		...
		protected override void OnDropAccepted(Object o, Vector2 mousePos){
			...
			if (o is GameObject){
				...
				foreach(var component in go.GetComponents<Component>().Where(c => c.hideFlags == 0)){
					...
					//wrappers
					foreach(var _cWrapperType in CustomObjectWrapper.FindCustomObjectWrappersForType( cType )){
						var cWrapperType = _cWrapperType;
						menu.AddItem( new GUIContent( string.Format(category + "Add Wrapper ({0})", cWrapperType.FriendlyName()) ), false, (x)=>{ this.AddObjectWrapper(cWrapperType, mousePos, null, (UnityEngine.Object)x);}, component );
					}
