...
namespace NodeCanvas.Framework{
	...
	partial class Connection{
		...
		bool IsPositionAlongConnection(Vector2 from, Vector2 to, Vector2 targetPosition){
			if ( ParadoxNotion.RectUtils.GetBoundRect(from, to).Contains(targetPosition) ){
				...
				for (var i = 0f; i <= CLICK_CHECK_RES; i++){
					var checkPos = GetPosAlongConnectionCurve(from, to, i/CLICK_CHECK_RES );
			...
			if (NCPrefs.connectionStyle != NCPrefs.ConnectionStyle.Stepped){
				if (status == Status.Running || isBlinking){
					for (var i = 0f; i < STATUS_BLINK_PACKET_COUNT; i++){
						...
						var norm = Mathf.Clamp01( Mathf.Repeat(traverse, 1f) );
						var pingPong = Mathf.PingPong(norm, 0.5f);
						var invSize = Mathf.InverseLerp(0.2f, 0.8f, pingPong );

						var packetSize = STATUS_BLINKPACKET_SIZE;
						packetSize *= Mathf.Lerp(1, 1.5f, invSize);

						var packetColor = this.color;
						packetColor.a = Mathf.Lerp(0, 2, pingPong);
						...
						rect.center = GetPosAlongConnectionCurve(lineFrom, lineTo, norm);;
