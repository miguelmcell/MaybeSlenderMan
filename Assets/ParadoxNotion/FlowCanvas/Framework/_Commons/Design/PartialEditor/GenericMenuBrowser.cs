		...
		void DoTree(Rect treeRect, Event e){
			...
			if (search != lastSearch){
				...
				if (!string.IsNullOrEmpty(search)){
					...
					foreach (var node in leafNodes.OrderBy(n => StringUtils.ScoreSearchMatch(search, n.parent != null? n.parent.fullPath : null )) ) {
						if (filterFavorites && !node.isFavorite){
							continue;
						}
						if (StringUtils.SearchMatch(search, node.fullPath, true)){
